...
import { Calendar, MapPin, DollarSign, MessageCircle, Video, Crown } from 'lucide-react';
...
export function JobCard({ job, onEdit, onNavigateToJobDetails }: Readonly<JobCardProps>) {
  ...
  useEffect(() => {
    const fetchInterviews = async () => {
      if (!job.id) return;
      try {
        const data = await getJobInterviews(Number.parseInt(job.id, 10));
        setInterviews(data);
      } catch (error) {
        ...
        {interviewScheduled && (
          <div className="p-2 bg-green-50 border border-green-200 rounded text-xs">
            ...
            <p className="text-gray-700">
              {new Date(interviewScheduled.scheduled_date).toLocaleDateString()} at {interviewScheduled.scheduled_time}
            </p>
