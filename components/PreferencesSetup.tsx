...
import { Check, MapPin, DollarSign, Briefcase, Home, Info, ChevronDown } from 'lucide-react';
...
export const PreferencesSetup: React.FC<PreferencesSetupProps> = ({
  ...
  const handleContinue = () => {
    const preferences: JobPreferences = {
      locations: selectedLocations.length > 0 ? selectedLocations : ['No Preference'],
      salaryRanges: selectedSalaryRanges.length > 0 ? selectedSalaryRanges : ['No Preference'],
      jobTypes: selectedJobTypes.length > 0 ? selectedJobTypes : ['No Preference'],
      workArrangements: selectedWorkArrangements.length > 0 ? selectedWorkArrangements : ['No Preference']
    };
        ...
        <div className="bg-white rounded-2xl shadow-xl border border-slate-200 p-8">
          ...
          <div className="flex items-center justify-between mt-8 pt-8 border-t border-slate-200">
            {onSkip && (
              <button
                onClick={onSkip}
                className="px-6 py-3 rounded-lg border-2 border-slate-300 text-slate-700 hover:bg-slate-50 hover:border-slate-400 transition-all"
              >
                Skip for now (Keep all preferences open)
              </button>
            )}
            <div className={onSkip ? '' : 'ml-auto'}>
              <button
                onClick={handleContinue}
                className="px-8 py-3 rounded-lg transition-all bg-[#ff6b35] text-white hover:bg-[#e55a2b] shadow-lg shadow-orange-500/30"
              >
